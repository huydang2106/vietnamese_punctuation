...
class AttentionCell(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        # Late fusion
        lfc = tf.compat.v1.layers.dense(w_context, self.num_units, use_bias=False, name='wfc')  # late fused context

        fw = tf.sigmoid(tf.compat.v1.layers.dense(lfc, self.num_units, use_bias=False, name='wff') +
                        tf.compat.v1.layers.dense(h, self.num_units, name='wfh')) # fusion weights
