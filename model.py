...
def dataset_batch_iter(dataset, batch_size):
    ...
    for record in dataset:
        ...
        batch_labels.append(record.get("labels",None))
...
def get_infer_dict(text:str,keep_number=True,lowercase=True,max_len_seq = 100):
    words = []
    ...
    for word in text.split():
        ...
        if len(words) > max_len_seq:
            ...
            break
        if len(words) == max_len_seq:
            return {"words": words,"labels": []}
            
    return {"words": words, "labels": []}

def restore_text(words:List[str],punctua_ls:List[int])->str:
    new_words_ls = []
    assert len(punctua_ls) >= len(words)
    for ind,word in enumerate(words):
        new_words_ls.append(word + PUNCTUATION_SYM[punctua_ls[ind]])
    return ' '.join(new_words_ls)
def infer(model,text:str)->str:
    ...
    tmp_ls = build_dataset(tmp_ls,model.word_dict,model.char_dict,model.label_dict)
    # print('after build dataset',tmp_ls)
    tmp_ls = batchnize_dataset(tmp_ls)
    # print('after batch nize',tmp_ls)
    predicts = model._predict_op(tmp_ls[0])
    # print(predicts)
...
#        self.loss = tf.reduce_mean(self.loss)
        ...
        self.train_op = optimizer.minimize(self.loss,var_list=tf.compat.v1.trainable_variables(),tape=tf.GradientTape())
